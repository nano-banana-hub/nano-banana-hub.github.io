...
function buildLlmsTxt(catalog) {
  const lines = [
    ...
    '- For image API setup, collect provider fields in the agent session when the user chooses direct entry and persist API keys with config quickset --api-key-stdin, or provide a local config quickset command with placeholders.',
    '- Do not assume an agent can run init --wizard; it is a human-terminal fallback.',
